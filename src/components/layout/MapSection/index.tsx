...
const MapSection = () => {
  ...
  const createMarkerIcon = useCallback(
    (content: string, label: string, size: [number, number] = [60, 80]) => {
      return L.divIcon({
        ...
        html: `
        <div class="flex flex-col items-center relative scale-75">
          ${content}
          <div class="mb-2.5">
          ...
          ${label === 'Madhya pradesh' ? `<img src="/gifs/finger-click.gif" class="absolute -top-10 left-[50px] h-[250px] w-[125px] -translate-x-1/2 scale-[40%] -rotate-10" />` : ''}
        </div>
      `,
        iconSize: size,
      });
        ...
        marker
          ...
          .bindPopup(
            `<div class="flex flex-col gap-3">
              <div class="flex justify-between leading-[34px] -tracking-[0.96px] font-dm-sans text-xl">
  ...
  const backToState = useCallback(() => {
    ...
    const minZoom = map.getBoundsZoom(stateGeoJsonLayer.getBounds()) - 2;
    map.setMinZoom(minZoom);
    map.setMaxBounds(stateGeoJsonLayer?.getBounds());
    map.flyToBounds(stateGeoJsonLayer.getBounds(), {
      padding: [150, 150],
      animate: true,
      duration: 0.7,
    });
  ...
  const backToIndia = useCallback(() => {
    ...
    map.flyToBounds(indiaGeoJsonLayer.getBounds(), {
      padding: [150, 150],
      animate: true,
      duration: 0.7,
    });
  ...
  const loadStateGeoJSONData = useCallback(
    async (cityName: string) => {
      const { map, roadwayLayer } = mapRefsRef.current;
      if (!map) return;
